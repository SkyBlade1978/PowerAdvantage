...
public class PowerAdvantage
{
	...
	public static final String VERSION = "2.4.3.110021";
	...
	*/
	...
	private static final String DEFAULT_DISTILLER_RECIPES = "2*crude_oil->1*refined_oil";
	private static final String LEGACY_DEFAULT_DISTILLER_RECIPES = "2*crude_oil->1*refined_oil;2*oil->1*refined_oil";
	...
	public void preInit(FMLPreInitializationEvent event)
	{
		...
		if (LEGACY_DEFAULT_DISTILLER_RECIPES.equals(distillerRecipeConfig)) {
			distillerRecipeConfig = DEFAULT_DISTILLER_RECIPES;
		}
		distillRecipes = distillerRecipeConfig.split(";");
		...
		zone.moddev.mc.poweradvantage.init.Fluids.init();
		zone.moddev.mc.poweradvantage.init.Blocks.init();
		zone.moddev.mc.poweradvantage.init.Items.init();
		zone.moddev.mc.poweradvantage.init.OreSpawnWorldGen.register(
				event.getSuggestedConfigurationFile().toPath().getParent());
		zone.moddev.mc.poweradvantage.init.TreasureChests.init(event.getSuggestedConfigurationFile().toPath().getParent());
	...
	private void clientPreInit(FMLPreInitializationEvent event){
		// client-only code
		zone.moddev.mc.poweradvantage.init.Blocks.bakeModels();
	}
	...
	public void init(FMLInitializationEvent event)
	{
		...
		zone.moddev.mc.poweradvantage.init.Fuels.init();
		zone.moddev.mc.poweradvantage.init.Entities.init();
		zone.moddev.mc.poweradvantage.init.Recipes.init();
		zone.moddev.mc.poweradvantage.init.Recipes.initDistillationRecipes(distillRecipes);
		zone.moddev.mc.poweradvantage.init.Recipes.initCrudeOilAliasDistillationRecipes();
		zone.moddev.mc.poweradvantage.init.Villages.init();
		zone.moddev.mc.poweradvantage.init.GUI.init();

		FMLLog.info("%s: mod support data registries", MODID);
		zone.moddev.mc.poweradvantage.init.ModSupport.init(detectedRF,detectedTechReborn);
	...
	private void clientInit(FMLInitializationEvent event){
		...
		zone.moddev.mc.poweradvantage.init.Items.registerItemRenders(event);
		zone.moddev.mc.poweradvantage.init.Blocks.registerItemRenders(event);
		zone.moddev.mc.poweradvantage.init.ModSupport.registerItemRenders(event);
	}
	@SideOnly(Side.SERVER)
